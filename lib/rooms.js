...
var listenersKeyPattern = 'room:%s:listeners';
...
function addRoomListener(roomId, socketId) {
	return client.sadd(util.format(listenersKeyPattern, roomId), socketId);
}

function removeRoomListener(socketId, callback) {
	callback = callback || function noop() {};
	client.KEYS(util.format(listenersKeyPattern, '*'), removeSocketFromListeners);
...
function init(resetListeners, callback) {
	...
	allRooms(function readRooms(err, rooms) {
		...
		rooms = rooms || [];
		async.map(rooms, function resetListenerCount(room, cb) {
			client.del(util.format(listenersKeyPattern, room.id), cb)
		}, function() {});
...
module.exports = exports = {
	...
	addListener: addRoomListener,
	removeListener: removeRoomListener
};
